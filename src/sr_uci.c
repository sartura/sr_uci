...
int uci_del(sr_ctx_t *ctx, const char *uci) {
  ...
  UCI_CHECK_RET(uci_ret, &rc, cleanup, "uci_lookup_ptr %d, path %s", uci_ret,
                uci);
  ...
  UCI_CHECK_RET(uci_ret, &rc, cleanup, "UCI save error %d, path %s", uci_ret,
                uci);

  uci_ret = uci_commit(ctx->uctx, &ptr.p, 1);
  UCI_CHECK_RET(uci_ret, &rc, cleanup, "UCI commit error %d, path %s", uci_ret,
                uci);
...
int get_uci_item(struct uci_context *uctx, char *ucipath, char **value) {
  ...
  UCI_CHECK_ITEM(ptr.o->v.string, &rc, cleanup, "Uci item %s not found",
                 ucipath);
...
/* manage uci sections */
int uci_section_cb(sr_ctx_t *ctx, char *xpath, char *ucipath,
                   sr_edit_flag_t flag, void *data) {
  return SR_ERR_OK;
}

/* manage uci options */
int uci_option_cb(sr_ctx_t *ctx, char *xpath, char *ucipath,
                  sr_edit_flag_t flag, void *data) {
  int rc = SR_ERR_OK;
  int uci_ret = UCI_OK;
  char *uci_val = NULL;
...
/* manage uci list's */
int uci_list_cb(sr_ctx_t *ctx, char *xpath, char *ucipath, sr_edit_flag_t flag,
                void *data) {
  int rc = SR_ERR_OK;
  int uci_ret = UCI_OK;
...
int uci_boolean_cb(sr_ctx_t *ctx, char *xpath, char *ucipath,
                   sr_edit_flag_t flag, void *data) {
  int rc = SR_ERR_OK;
  int uci_ret = UCI_OK;
  char *uci_val = NULL;
...
/* manage uci boolean values but reverse them in sysrepo, used for ignore
 * options */
int uci_boolean_reverse_cb(sr_ctx_t *ctx, char *xpath, char *ucipath,
                           sr_edit_flag_t flag, void *data) {
  int rc = SR_ERR_OK;
  int uci_ret = UCI_OK;
  char *uci_val = NULL;
...
int sr_option_cb(sr_ctx_t *ctx, sr_change_oper_t op, sr_val_t *old_val,
                 sr_val_t *new_val, char *xpath, char *ucipath) {
  int rc = SR_ERR_OK;
...
int sr_boolean_reverse_cb(sr_ctx_t *ctx, sr_change_oper_t op, sr_val_t *old_val,
                          sr_val_t *new_val, char *xpath, char *ucipath) {
  int rc = SR_ERR_OK;
...
int sr_boolean_cb(sr_ctx_t *ctx, sr_change_oper_t op, sr_val_t *old_val,
                  sr_val_t *new_val, char *xpath, char *ucipath) {
  int rc = SR_ERR_OK;
...
int sr_section_cb(sr_ctx_t *ctx, sr_change_oper_t op, sr_val_t *old_val,
                  sr_val_t *new_val, char *xpath, char *ucipath) {
  int rc = SR_ERR_OK;
...
/* sysrepo callback for writing leaf-lists to uci, every list element is
 * directly add/deleted in uci */
int sr_list_cb(sr_ctx_t *ctx, sr_change_oper_t op, sr_val_t *old_val,
               sr_val_t *new_val, char *xpath, char *ucipath) {
  int rc = SR_ERR_OK;
  int uci_ret = UCI_OK;
  struct uci_ptr ptr = {};
  ...
  if (SR_OP_DELETED == op || SR_OP_MODIFIED == op) {
    ...
    UCI_CHECK_RET(uci_ret, &rc, cleanup, "lookup_pointer %d %s", uci_ret,
                  set_path);
...
size_t sr_value_node_size(struct list_head *list) {
  ...
  list_for_each_entry(tmp, list, head) { current_size += 1; }
...
int sr_value_node_copy(struct list_head *list, sr_val_t **values,
                       size_t *values_cnt) {
...
int ubus_string_to_sr(struct list_head *list, struct json_object *top,
  ...
  CHECK_NULL(jobj, &rc, cleanup, "json_object_object_get_ex: failed on %s",
...
int ubus_uint8_to_sr(struct list_head *list, struct json_object *top,
                     char *ubus_obj_name, char *xpath) {
  uint8_t jobj_int = 0;
  struct json_object *jobj = NULL;
  sr_value_node_t *list_value = NULL;
...
int sysrepo_to_uci(sr_ctx_t *ctx, sr_change_oper_t op, sr_val_t *old_val,
                   sr_val_t *new_val, sr_event_t event) {
  char *xpath = NULL;
  char *ucipath = NULL;
...
int sr_uci_init_data(sr_ctx_t *ctx, const char *uci_config,
  ...
  if (UCI_ERR_DUPLICATE != uci_ret) {
    UCI_CHECK_RET(uci_ret, &rc, cleanup,
                  "No configuration (package) %s, uci_error %d", uci_config,
                  uci_ret);
  }
  ...
  uci_foreach_element(&ctx->package->sections, e) {
    ...
    while (*section != 0) {
      if (string_eq(s->type, (char *)*section)) {
        ...
        CHECK_RET(rc, cleanup, "failed to add sysrepo data: %s",
                  sr_strerror(rc));
      }
      section++;
    }
